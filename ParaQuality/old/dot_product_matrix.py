...
def create_model(shape, filters, print_summary=False, compile=True):
    ...
    input = dot([input_1, input_2], axes=-1)

    conv = Conv1D(filters=filters, kernel_size=4, data_format="channels_first")
    conv_out_1 = conv(input)
    maxp_1 = MaxPool1D(pool_size=16)(conv_out_1)
    flat_1 = Flatten()(maxp_1)

    conv2 = Conv1D(filters=filters, kernel_size=4, data_format="channels_last")
    conv2_out_1 = conv2(input)
    maxp_2 = MaxPool1D(pool_size=12)(conv2_out_1)
    flat_2 = Flatten()(maxp_2)
